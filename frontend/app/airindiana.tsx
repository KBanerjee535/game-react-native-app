...
const isOnLand = (x_norm: number, y_norm: number): boolean => {
  const mask = LAND_MASKS['airindiana'];
...
// === COMPONENT ===
export default function AirIndianaScreen() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const prev = await AsyncStorage.getItem('best_airindiana');
        const best = prev ? parseInt(prev, 10) : 0;
        if (delivered > best) {
          await AsyncStorage.setItem('best_airindiana', String(delivered));
        }
      } catch (e) {
        // ignore
      }
    ...
    <View style={styles.container} onLayout={onLayout}>
      ...
      {mapW > 0 && mapH > 0 && (
        <View style={{ position: 'absolute', left: imgRect.x, top: imgRect.y, width: imgRect.w, height: imgRect.h, overflow: 'hidden' }}>
          <Image
            source={require('../assets/images/airindiana-map.png')}
        ...
        <View style={{ flex: 1, backgroundColor: "#000000" }}>
          ...
          <View style={styles.modalFullContent}>
            ...
            {delivered >= 1500 && (
              <View style={styles.modalFullUnlockGroup}>
                ...
                <View style={styles.modalFullUnlockCard}>
                  <Image
                    source={require('../assets/images/antartikair-logo.png')}
            ...
            <View style={styles.modalFullButtonsRow}>
              ...
              {delivered >= 1500 && (
                <Pressable style={[styles.modalFullBtn, { backgroundColor: '#5C8D3F' }]} onPress={() => router.replace('/antartikair')}>
