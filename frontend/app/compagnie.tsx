...
const isOnLand = (x_norm: number, y_norm: number): boolean => {
  const mask = LAND_MASKS['compagnie'];
...
export default function CompagnieScreen() {
  ...
  const restartGame = useCallback(() => {
    ...
    setLines([]);
    setPlanes([]);
    setMiles(STARTING_MILES);
    setDelivered(0);
    setCollected(0);
    setCargoUnlocked(false);
    setHubUnlocked(false);
    setShowCargoUnlockPopup(false);
    setShowHubUnlockPopup(false);
    setCollisionPopup(null);
    setSelectedDestId(null);
    setGameOver(null);
    setShowInsufficientMiles(false);
    setLockedDestMsg(null);
    setUiMode('idle');
        ...
        const updated = prevPlanes.map(p => {
          ...
          if (newProg >= 1) {
            ...
            if (arriveDest) {
              ...
              // Account for any pickups already performed at this dest THIS TICK
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const prev = await AsyncStorage.getItem('best_air_atlante');
        const best = prev ? parseInt(prev, 10) : 0;
        if (delivered > best) {
          await AsyncStorage.setItem('best_air_atlante', String(delivered));
        }
      } catch (e) {
        // ignore
      }
    ...
    <View style={styles.container} onLayout={onLayout}>
      ...
      {mapW > 0 && mapH > 0 && (
        <View style={{ position: 'absolute', left: imgRect.x, top: imgRect.y, width: imgRect.w, height: imgRect.h, overflow: 'hidden' }}>
          <Image
            source={require('../assets/images/compagnie-map.png')}
        ...
        <View style={{ flex: 1, backgroundColor: "#000000" }}>
          ...
          <View style={styles.modalFullContent}>
            ...
            {delivered >= 500 && (
              <View style={styles.modalFullUnlockGroup}>
                ...
                <View style={styles.modalFullUnlockCard}>
                  <Image
                    source={require('../assets/images/pacifikair-logo.png')}
            ...
            <View style={styles.modalFullButtonsRow}>
              ...
              {delivered >= 500 && (
                <Pressable style={[styles.modalFullBtn, { backgroundColor: '#5C8D3F' }]} onPress={() => router.replace('/pacifikair')}>
