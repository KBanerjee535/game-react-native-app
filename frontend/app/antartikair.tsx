...
interface Destination {
  ...
  mails: number;
  isHub?: boolean;
  isInternational?: boolean; // aéroport international (AIR INDIANA only)
}
...
interface Line {
  ...
  color: string;
  isInternational?: boolean; // ligne internationale (épaisse, peut croiser)
}
...
const isOnLand = (x_norm: number, y_norm: number): boolean => {
  const mask = LAND_MASKS['antartikair'];
...
// === COMPONENT ===
export default function AntartikairScreen() {
  ...
  // Mode UI : 'idle' | 'creating_line' | 'extending_*' | 'cargo_pick_line' | 'hub_pick_dest' | 'intl_pick_dest' | 'intl_creating_line'
  const [uiMode, setUiMode] = useState<'idle' | 'creating_line' | 'extending_pick_line' | 'extending_pick_endpoint' | 'extending_pick_dest' | 'cargo_pick_line' | 'hub_pick_dest' | 'intl_pick_dest' | 'intl_creating_line_from' | 'intl_creating_line_to'>('idle');
  ...
  useEffect(() => {
    ...
    const id = setInterval(() => {
      ...
      // À partir de 1000 courriers collectés : production +30% (intervalle réduit à 70%)
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const prev = await AsyncStorage.getItem('best_antartikair');
        const best = prev ? parseInt(prev, 10) : 0;
        if (delivered > best) {
          await AsyncStorage.setItem('best_antartikair', String(delivered));
        }
      } catch (e) {
        // ignore
      }
  ...
  const intlAirportCount = destinations.filter(d => d.isInternational).length;
  const MAX_CARGO = 3;
  const MAX_HUB = 5;
  const MAX_INTL = 3;
  const intlUnlocked = collected >= 800;
  ...
  const handleIntlAirportBtn = () => {
    if (gameOver) return;
    if (uiMode !== 'idle') { cancelMode(); return; }
    if (intlAirportCount >= MAX_INTL) {
      setLockedDestMsg(`Limite de ${MAX_INTL} aéroports internationaux atteinte.`);
      return;
    }
  ...
  const handleDestTap = (destId: string) => {
    ...
    if (uiMode === 'intl_pick_dest') {
      // User picks a destination to upgrade into INTERNATIONAL AIRPORT (also a HUB)
      const dest = destinations.find(d => d.id === destId);
      if (!dest) return;
      if (dest.isInternational) return; // already international
      if (miles < COST_INTL) { setShowInsufficientMiles(true); cancelMode(); return; }
      setMiles(m => m - COST_INTL);
      setDestinations(prev => prev.map(d => d.id === destId ? { ...d, isHub: true, isInternational: true } : d));
      // Now switch to international line creation mode (start from this dest)
      setSelectedDestId(destId);
    ...
    if (uiMode === 'intl_creating_line_to') {
      // User picks the second dest for an international line (crosses anything, has cargo)
      if (!selectedDestId) { cancelMode(); return; }
      if (destId === selectedDestId) { setSelectedDestId(null); setUiMode('idle'); return; }
      const fromD = destinations.find(d => d.id === selectedDestId);
      const toD = destinations.find(d => d.id === destId);
      if (!fromD || !toD) { cancelMode(); return; }
      // Cost is just extension (already paid INTL when upgrading)
      ...
      // Spawn a CARGO plane immediately on this international line
      setPlanes(prev => [...prev, {
        id: `plane_${newLineId}`,
        lineId: newLineId,
        segIndex: 0,
    ...
    <View style={styles.container} onLayout={onLayout}>
      ...
      {mapW > 0 && mapH > 0 && (
        <View style={{ position: 'absolute', left: imgRect.x, top: imgRect.y, width: imgRect.w, height: imgRect.h, overflow: 'hidden' }}>
          <Image
            source={require('../assets/images/antartikair-map.png')}
      ...
      <View style={styles.bottomBar}>
        ...
        {(cargoUnlocked || hubUnlocked || intlUnlocked) && (
          <View style={[styles.bottomRow, { marginTop: 6 }]}>
            ...
            {intlUnlocked && (
              <Pressable
                style={[styles.actionBtn, (uiMode === 'intl_pick_dest' || uiMode === 'intl_creating_line_to') && styles.actionBtnActive, intlAirportCount >= MAX_INTL && styles.actionBtnDisabled]}
                onPress={handleIntlAirportBtn}
              >
                <MaterialCommunityIcons name="airplane" size={18} color={intlAirportCount >= MAX_INTL ? '#888' : '#FFD700'} />
                <View>
                  <Text style={[styles.actionBtnLabel, intlAirportCount >= MAX_INTL && { color: '#888' }]}>{t('COMP_AEROPORT_INTL')} {intlAirportCount}/{MAX_INTL}</Text>
                  <Text style={[styles.actionBtnCost, intlAirportCount >= MAX_INTL && { color: '#888' }]}>50 000 MILES</Text>
                </View>
              </Pressable>
            )}
          </View>
        ...
        <View style={{ flex: 1, backgroundColor: "#000000" }}>
          ...
          <View style={styles.modalFullContent}>
            ...
            {delivered >= 2000 && (
              <View style={styles.modalFullUnlockGroup}>
                ...
                <View style={styles.modalFullUnlockCard}>
                  <Image
                    source={require('../assets/images/worldwide-logo.png')}
            ...
            <View style={styles.modalFullButtonsRow}>
              <Pressable style={styles.modalFullBtn} onPress={() => router.replace('/antartikair')}>
                <Text style={styles.modalFullBtnText}>{t('REJOUER')}</Text>
              </Pressable>
              {delivered >= 2000 && (
                <Pressable style={[styles.modalFullBtn, { backgroundColor: '#5C8D3F' }]} onPress={() => router.replace('/worldwide')}>
